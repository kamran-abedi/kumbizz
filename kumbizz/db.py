...
def register_invite(invited, inviter):
    with conn:
        ...
        cursor.execute("UPDATE invites SET invite_count = invite_count + 1 WHERE inviter_id=?", (inviter,))
...
def increment_invite_count(inviter):
    with conn:
        conn.execute("UPDATE invites SET invite_count = invite_count + 1 WHERE inviter_id=?", (inviter,))
...
def get_invite_count(telegram_id):
    with conn:
        cursor = conn.cursor()
        cursor.execute("SELECT invite_count FROM invites WHERE inviter_id=?", (telegram_id,))
        row = cursor.fetchone()
